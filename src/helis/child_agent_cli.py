...
def materialize(
    ...
    console.print(
        f"venture={report.opportunity_id} agents={len(report.artifacts)} created={report.created_count}"
    )
...
def smoke(
    ...
    """FIRST BOOT diagnostic using the real Factory + Runtime and the configured local model."""
    with TemporaryDirectory(prefix="helis-first-boot-") as temporary:
        ...
        if report.blocked_reason is not None or len(report.artifacts) != 1:
            raise RuntimeError(f"smoke factory failed: {report.blocked_reason or 'unexpected count'}")
        artifact = report.artifacts[0]
        console.print(
            f"[bold green]FIRST BOOT[/] artifact={artifact.id} capability={artifact.capability_key}"
        )
        budget = CycleBudget(max_model_calls=max_model_calls, max_tokens=12_000, max_cost_cents=5.0)
        result = ChildAgentRuntime(
            engine,
            OpenAICompatibleProvider.from_env(),
            budget,
