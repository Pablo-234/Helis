...
class ChildAgentFactory:
    ...
    def materialize_if_needed(self, opportunity_id: UUID) -> ChildAgentFactoryReport:
        ...
        if bundle is None:
            return ChildAgentFactoryReport(
                opportunity_id,
                [],
                blocked_reason="agent_specs_missing",
            )
        ...
        if (
            ...
            return ChildAgentFactoryReport(
                opportunity_id,
                [],
                blocked_reason="agent_specs_stale",
            )
    ...
    def _materialize(self, bundle: AgentSpecBundle, spec: ChildAgentSpec) -> ChildAgentArtifact:
        ...
        relative = (
            Path(str(bundle.opportunity_id))
            / "agents"
            / spec.capability_key
            / artifact_hash
            / "agent.json"
        ...
        if target.exists():
            ...
            if actual != artifact_hash:
                raise ChildAgentArtifactTampered(
                    f"pre-existing child-agent path is not exact: {target}"
                )
        else:
    ...
    def _target_from_relative(self, relative: str) -> Path:
        ...
        try:
            ...
            raise ChildAgentArtifactTampered(
                "child-agent path escapes venture workspace"
            ) from exc
        return target
