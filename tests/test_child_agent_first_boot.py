...
def test_reasoning_only_runtime_completes_and_persists_venture_run(tmp_path) -> None:
    ...
    provider = QueueProvider(
        [
            {
                "state": "completed",
                "output": "1. Ask for budget. 2. Ask for deadline. 3. Then prepare the next action.",
                "next_step": None,
                "needs_tool": None,
            }
        ]
    ...
    assert any(event.event_type == "venture.child_agent_run" for event in engine.store.list_events())
...
def test_runtime_enforces_spec_turn_cap(tmp_path) -> None:
    ...
    provider = QueueProvider(
        [
            {"state": "continue", "output": "first", "next_step": "second", "needs_tool": None},
            {"state": "continue", "output": "second", "next_step": "third", "needs_tool": None},
        ]
    )
